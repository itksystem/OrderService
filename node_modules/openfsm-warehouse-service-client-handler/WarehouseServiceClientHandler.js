...
async productAvailability(token) { // возвращает инфлормацию о доступности товаров на складе
    try {
        const response = await fetch(process.env.WAREHOUSE_SERVICE_PRODUCT_AVAILABILITY_URL, {                
            method: 'GET',                
            headers: { 'Content-Type' : 'application/json', 'Authorization': `Bearer ${token}`, }
 ...
 async getOrderDetails(token, orderId) {
    try {
        const url = new URL(`${process.env.WAREHOUSE_SERVICE_ORDER_DETAILS_URL}/${orderId}/details`);
        
        const controller = new AbortController();
        const timeoutId = setTimeout(() => controller.abort(), 5000); // Тайм-аут 5 сек
        ...
        return { success: false, status: error.status || 500, message: error.message || 'Internal Server Error' };
    }
}
